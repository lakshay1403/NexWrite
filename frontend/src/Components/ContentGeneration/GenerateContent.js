...
import { GenerateContentAPI } from "../../apis/Gemini/GenrateApi";
 ...
 // Formik setup for handling form data
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      console.log(values);
      mutation.mutate(`Generate a blog post based ${values.prompt}, 
        ${values.category}, ${values.tone} `);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        {/* Success */}
        {mutation?.isSuccess && (
            <StatusMessage type='success' message='Content generation is successfull'/>
        )}
        ...
        <div className="flex">
            <div className="mr-2 mb-2">
                <span className="text-sm font-semibold bg-green-200 px-4 py2 rounded-full">Plan: {data?.user?.subscriptionPlan}</span>
            </div>
        </div>
        ...
        <div className="flex">
            <div className="mr-2 mb-2">
                <span className="text-sm font-semibold bg-green-200 px-4 py2 rounded-full">Credit: {data?.user?.apiRequestCount} / {" "} {data?.user?.monthlyRequestCount}</span>
            </div>
        </div>
        ...
        {generatedContent && (
          <div className="mt-6 p-4 bg-gray-100 rounded-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">
              Generated Content: {mutation?.data?.message}
            </h3>
